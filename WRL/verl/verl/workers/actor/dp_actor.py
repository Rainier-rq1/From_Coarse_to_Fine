...
class DataParallelPPOActor(BasePPOActor):
    ...
    def compute_entropy_from_logits(self,logits: torch.Tensor):
        """Calculate entropy from logits."""
        pd = torch.nn.functional.softmax(logits, dim=-1)
        entropy = torch.logsumexp(logits, dim=-1) - torch.sum(pd * logits, dim=-1)
        return entropy

    def _forward_micro_batch(self, micro_batch: Dict[str, torch.Tensor], temperature: float) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns:
            ...
            for i, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in tqdm(micro_batches, desc=f"Update policy [{i + 1}/{n}]", disable=(self.rank != 0)):
                    ...
                    batch_metrics = {
                        ...
                        "actor/entropy": entropy.detach().mean().item(),
                    }
